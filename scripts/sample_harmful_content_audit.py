...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "write harmful-content classifier outputs for manual audit."
        )
    )
    parser.add_argument(
        "warc_paths",
...
def reservoir_sample_documents(
    ...
    stats = {
        ...
        "num_nsfw_positive": 0,
        "num_toxic_positive": 0,
        "num_any_harmful_positive": 0,
    }
    ...
    for warc_path, record_index, uri, html_bytes in iter_response_records(warc_paths):
        ...
        nsfw_label, nsfw_score = classify_nsfw(extracted_text)
        toxic_label, toxic_score = classify_toxic_speech(extracted_text)

        if nsfw_label == "nsfw":
            stats["num_nsfw_positive"] += 1
        if toxic_label == "toxic":
            stats["num_toxic_positive"] += 1
        if nsfw_label == "nsfw" or toxic_label == "toxic":
            stats["num_any_harmful_positive"] += 1
...
def main() -> None:
        ...
        prediction_rows.append(
            {
                ...
                "nsfw_score": doc["nsfw_score"],
                "toxic_label": doc["toxic_label"],
                "toxic_score": doc["toxic_score"],
                "any_harmful_label": "harmful"
                if doc["nsfw_label"] == "nsfw" or doc["toxic_label"] == "toxic"
                else "non-harmful",
            }
        )
    ...
    summary = {
        ...
        **stats,
        "fractions": {
            "nsfw_positive_fraction": stats["num_nsfw_positive"] / num_eligible if num_eligible else 0.0,
            "toxic_positive_fraction": stats["num_toxic_positive"] / num_eligible if num_eligible else 0.0,
            "any_harmful_positive_fraction": stats["num_any_harmful_positive"] / num_eligible if num_eligible else 0.0,
        },
    }
    ...
    print(
        "Classifier stats: "
        f"{stats['num_nsfw_positive']} NSFW positives, "
        f"{stats['num_toxic_positive']} toxic positives, "
        f"{stats['num_any_harmful_positive']} harmful-by-either positives"
    )
